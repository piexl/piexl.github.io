...
(function ($) {      
	...
	function resetIndex(){
		...
		function getNextDay(d){
			...
			d = +d + 1000*60*60*24;
			d = new Date(d);
			if((d.getMonth()+1) < 10 ){ var oMonth = "0"+(d.getMonth()+1);}else{var oMonth = d.getMonth()+1;}
			//if(d.getDate() < 10){ var oDate = "0"+d.getDate();}else{var oDate = d.getDate();}
			return d.getFullYear()+"/"+oMonth+"/"+d.getDate();
		}
            ...
            $("#dateconfirm").unbind('click').click(function () {
			  ...
			  $("#daywrapper ul li:eq("+Math.round(indexD)+")").html().substr(0,$("#daywrapper ul li:eq("+Math.round(indexD)+")").html().length-1);
               if(datetime){
                ...
                $("#dateshadow").hide();
				if(opts.start_c){
					var nextDate = getNextDay($('#beginTime').val());
					$('#stopTime').val(nextDate);
            ...
            var strM = $("#monthwrapper ul li:eq("+indexM+")").html().substr(0,$("#monthwrapper ul li:eq("+indexM+")").html().length-1)
              yearScroll = new iScroll("yearwrapper",{snap:"li",vScrollbar:false,
                  onScrollEnd:function () {
					   ...
					   $("#yearwrapper li").eq(indexY).addClass('current').siblings('li').removeClass('current');
              ...
              monthScroll = new iScroll("monthwrapper",{snap:"li",vScrollbar:false,
                  onScrollEnd:function (){
					  ...
					  $("#monthwrapper li").eq(indexM).addClass('current').siblings('li').removeClass('current');
                      opts.endday = checkdays(strY,strM);
							//$("#daywrapper ul").html(createDAY_UL());
                           //dayScroll.refresh();
                  }});
              dayScroll = new iScroll("daywrapper",{snap:"li",vScrollbar:false,
                  onScrollEnd:function () {
					  ...
					  $("#daywrapper li").eq(indexD).addClass('current').siblings('li').removeClass('current');
        ...
        function createYEAR_UL(){
            ...
            for(var i=opts.beginyear; i<=opts.endyear;i++){
				if(i == 2015){
					str+='<li class="current">'+i+'年</li>'
				}else{
					str+='<li>'+i+'年</li>'
				}
				//str+='<li class=c_'+i+'>'+i+'年</li>'
            }
            return str+"<li>&nbsp;</li>";;
        }
        //创建 --月-- 列表
